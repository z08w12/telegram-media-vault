...
function keyboard(event) {
  ...
  if (event.key === 'Escape') { selected.value = null; tagManagerOpen.value = false; albumManagerOpen.value = false; return; }
  if (!selected.value || editing) return; if (event.key === 'ArrowLeft') navigateImage(-1); if (event.key === 'ArrowRight') navigateImage(1);
}
...
onBeforeUnmount(() => { window.removeEventListener('keydown', keyboard); clearTimeout(searchTimer); });
</script>
...
<template>
  ...
  <div v-if="selected" class="modal" @click.self="selected = null"><button class="modal-close" @click="selected = null">×</button><button v-if="selected.kind === 'image'" class="image-nav previous" :disabled="selectedImageIndex <= 0" aria-label="上一张" @click="navigateImage(-1)">‹</button><button v-if="selected.kind === 'image'" class="image-nav next" :disabled="selectedImageIndex < 0 || selectedImageIndex >= imageItems.length - 1" aria-label="下一张" @click="navigateImage(1)">›</button><div class="viewer"><img v-if="selected.kind === 'image'" :src="`${API_BASE}/media/${selected.id}/content`" :alt="selected.original_name || ''" /><video v-else-if="selected.kind === 'video'" :src="`${API_BASE}/media/${selected.id}/content`" controls autoplay playsinline /><div v-else class="document-large">▤</div></div><aside class="details"><p class="eyebrow">{{ selected.kind }}</p><h2>{{ selected.original_name || `媒体 #${selected.id}` }}</h2><p v-if="selected.caption">{{ selected.caption }}</p><section class="media-tags"><h3>标签</h3><div class="tag-checks"><label v-for="tag in tags" :key="tag.id"><input v-model="selectedTagIds" type="checkbox" :value="tag.id" /><i :style="{ background: tag.color }"></i>{{ tag.name }}</label></div><button class="ghost" @click="saveMediaTags">保存标签</button></section><section class="media-tags"><h3>相册</h3><div class="tag-checks"><label v-for="album in albums" :key="album.id"><input v-model="selectedAlbumIds" type="checkbox" :value="album.id" />▣ {{ album.name }}</label></div><button class="ghost" @click="saveMediaAlbums">保存相册</button></section><dl><dt>大小</dt><dd>{{ formatBytes(selected.size_bytes) }}</dd><dt>转存时间</dt><dd>{{ formatDate(selected.created_at) }}</dd><dt v-if="selected.source_name">来源</dt><dd v-if="selected.source_name">{{ selected.source_name }}</dd></dl><div class="actions"><a class="primary button" :href="`${API_BASE}/media/${selected.id}/download`">下载原文件</a><button v-if="!trash" class="ghost" @click="toggleFavorite(selected)">{{ selected.is_favorite ? '取消收藏' : '加入收藏' }}</button><button v-if="!trash" class="danger" @click="moveToTrash(selected)">移至回收站</button><button v-if="trash" class="ghost" @click="restore(selected)">恢复</button><button v-if="trash" class="danger" @click="removeForever(selected)">永久删除</button></div></aside></div>
